...
void addRecord(struct Plate** head, const char* fileName, const char* permissions, size_t size) {



    struct Plate* newRecord = (struct Plate*)malloc(sizeof(struct Plate));
...
void file_split(int argc, char *argv[]) {
    ...
    struct Plate* currentDebug = organizationList;
...
int main(int argc, char *argv[]) {
   ...
   if (strcmp(argv[1], "-b") == 0) {
        ...
        file_merger(argc, argv);  // Uncomment this line if you want to implement mergeFiles

    } else if (strcmp(argv[1], "-a") == 0) {

        file_split(argc, argv);
